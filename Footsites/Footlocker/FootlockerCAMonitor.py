...
class FootlockerBot:
    ...
    def scrape_main_site(self):
        ...
        s = requests.Session()
        try:
            html = s.get('https://www.footlocker.ca/en/search?query=allcategories%3Arelevance%3AisNewarrival%3ANEW%2BARRIVALS&sort=newArrivals&currentPage=0', headers=self.headers, proxies=self.proxy, verify=False, timeout=10)
